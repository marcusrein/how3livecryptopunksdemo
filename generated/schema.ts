...
export class CallData extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save CallData entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type CallData must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("CallData", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): CallData | null {
    return changetype<CallData | null>(
      store.get_in_block("CallData", id.toHexString())
    );
  }

  static load(id: Bytes): CallData | null {
    return changetype<CallData | null>(store.get("CallData", id.toHexString()));
  }
